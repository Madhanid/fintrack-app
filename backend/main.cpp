...
std::string make_event(const std::string &type,
...
int main() {
  ...
  CROW_WEBSOCKET_ROUTE(app, "/ws")
      ...
      .onclose([&](crow::websocket::connection &conn, const std::string &,
        ...
        std::cout << "WS client disconnected. Total: " << ws_clients.size()
                  << std::endl;
      })
  ...
  CROW_ROUTE(app, "/api/<path>")
      .methods("OPTIONS"_method)([&](crow::response &res, const std::string &) {
        cors_response(res);
        res.code = 204;
        res.end();
      });
  ...
  ([&](crow::response &res) {
    ...
    double totalBalance = 0, totalIncome = 0, totalExpenses = 0,
           monthlyBudget = 0;
    for (const auto &b : budgets)
      monthlyBudget += b.target;
    for (const auto &t : txs) {
      totalBalance += t.amount;
      if (t.amount > 0)
        totalIncome += t.amount;
      else
        totalExpenses += std::abs(t.amount);
    }
  ...
  CROW_ROUTE(app, "/api/transactions")
      .methods(
        ...
        std::string date = x.has("date") ? std::string(x["date"].s())
                                         : std::string("2023-01-01");
        std::string category = x.has("category")
                                   ? std::string(x["category"].s())
                                   : std::string("General");

        bool success = db.addTransaction(desc, amount, date, category);
        if (success) {
          // Build the transaction payload for the broadcast
          std::string txPayload = "{\"description\":\"" + desc +
          ...
          for (const auto &t : txs) {
            ...
            if (t.amount > 0)
              ...
              exp += std::abs(t.amount);
          }
          std::string sumPayload = "{\"totalBalance\":" + std::to_string(bal) +
  ...
  CROW_ROUTE(app, "/api/transactions/<int>")
      .methods("PUT"_method)(
          [&](const crow::request &req, crow::response &res, int id) {
            ...
            bool success =
                db.updateTransaction(id, x["description"].s(), x["amount"].d(),
                                     x["date"].s(), x["category"].s());

            if (success) {
              ws_broadcast(make_event("transaction_updated",
                                      "{\"id\":" + std::to_string(id) + "}"));
            }
  ...
  CROW_ROUTE(app, "/api/transactions/<int>")
      .methods("DELETE"_method)([&](crow::response &res, int id) {
        ...
        if (success) {
          ws_broadcast(make_event("transaction_deleted",
                                  "{\"id\":" + std::to_string(id) + "}"));
      ...
      .methods(
        ...
        if (lowerMsg.find("analyze") != std::string::npos ||
          ...
          if (categorySpending.empty()) {
            aiResponse += "You haven't recorded any expenses yet! Start adding "
                          "some to get insights.";
          } else {
            auto topCat = std::max_element(categorySpending.begin(),
            ...
            aiResponse += "To reduce costs, try to limit non-essential "
                          "purchases in this category. ";
            if (totalSpend > 5000)
              aiResponse +=
                  "Also, your total monthly spending is quite high. "
          ...
          aiResponse = "Hello! I am your FinTrack AI assistant. How can I "
                       "assist you with your finances today?";
        } else if (lowerMsg.find("balance") != std::string::npos) {
          aiResponse = "Your total balance is determined by your total income "
                       "minus your expenses. "
                       "You can check the Dashboard for a real-time overview.";
        } else if (lowerMsg.find("budget") != std::string::npos) {
          aiResponse = "Setting budgets is a great way to save money! You have "
                       "active budgets in your dashboard. "
          ...
          aiResponse = "To save more money, start by identifying non-essential "
                       "expenses in categories like "
                       "Entertainment. Also, consider setting up a recurring "
                       "transfer to a savings account.";
        } else {
          aiResponse = "That's an interesting question! Based on your "
                       "financial data, I recommend watching "
                       "your expense trends. I am always learning to answer "
  ...
  std::cout << "FinTrack C++ Backend running on port 8080 (WebSocket + SSE "
               "enabled)..."
            << std::endl;
